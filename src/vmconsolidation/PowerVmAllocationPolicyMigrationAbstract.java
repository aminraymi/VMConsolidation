...
public abstract class PowerVmAllocationPolicyMigrationAbstract extends PowerVmAllocationPolicyAbstract {
    ...
    HashMap<String, RountingModel> routingTable;
     ...
     * @param vmType the type of the vm "HHH - LLL"
     * @return the power host
     */
    public PowerHost findHostForVm(Vm vm, Set<? extends Host> excludedHosts, String vmType) {
       
        
        
        PowerHost allocatedHost = null;
        int numberOfHosts = routingTable.get(vmType).getHosts().size();
        int counter = -1;
        while(true){
            counter++;
            int nextHost = routingTable.get(vmType).getNextHost();
            PowerHost host = routingTable.get(vmType).getHosts().get(nextHost);
            
            if(counter == numberOfHosts + 1){
                break;
            }
            if(excludedHosts.contains(host)){
                nextHost++;
                if(nextHost == numberOfHosts){
                    nextHost = 0;
                }
                routingTable.get(vmType).setNextHost(nextHost);
                continue;
            }
       ...
       /*
        ...
        List<PowerHost> powerHostList = this.<PowerHost>getHostList();
    ...
    protected List<Map<String, Object>> getNewVmPlacement(
        ...
        // get vm's ram utilization indexex in sorted by ram utilization desc order
        ArrayList<Integer> vmsRamUtilizationPositions = MyPowerVmList.sortByRamUtilization(vmsToMigrate);

        // get vm's bandwidth utilization indexex in sorted by bandwidth utilization desc order
        ArrayList<Integer> vmsBwUtilizationPositions = MyPowerVmList.sortByBwUtilization(vmsToMigrate);

        // get hosts
        List<PowerHost> sortByAvailabeMipsHostList = this.<PowerHost>getHostList();

        // sort hosts ba available mips in desc order
        MyPowerHostList.sortByAvailableCpu(sortByAvailabeMipsHostList);

        // get host's available ram position in sorted by ram availability desc order
        ArrayList<Integer> hostsRamUtilizationPositions = MyPowerHostList.sortByAvailableRam(sortByAvailabeMipsHostList);

        // get host's available bandwidth position in sorted by bandwidth availability desc order
        ArrayList<Integer> hostsBwUtilizationPositions = MyPowerHostList.sortByAvailableBw(sortByAvailabeMipsHostList);

        // split hosts into different types (CPU,RAM,BW) availability
        HashMap<String, ArrayList<PowerHost>> hostsTypes = PlacementUtils.getHostsTypesByResourceAvailability(sortByAvailabeMipsHostList, hostsRamUtilizationPositions, hostsBwUtilizationPositions);

        // create the routing table for vm placement
        routingTable = PlacementUtils.createRoutingTable(hostsTypes);

        int vmIndex = 0;
        for (Vm vm : vmsToMigrate) {

            String vmType = PlacementUtils.getVmTypeByUtilizations(vmsToMigrate, vmsRamUtilizationPositions, vmsBwUtilizationPositions, vmIndex);

            vmIndex++;

            PowerHost allocatedHost = findHostForVm(vm, excludedHosts, vmType);
