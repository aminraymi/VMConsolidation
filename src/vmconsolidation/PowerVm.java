...
public class PowerVm extends Vm {
    ...
    /**
     * The processor utilization history.
     */
     ...
     */
    private final List<Double> RamUtilizationHistory = new LinkedList<>();

    /**
     * The Bandwidth utilization history.
     */
    private final List<Double> BwUtilizationHistory = new LinkedList<>();
    ...
    public PowerVm(
            ...
            final int priority,
            final String vmm,
            final MyCloudletScheduler cloudletScheduler,
    ...
    public double getRamUtilizationMad() {
        double mad = 0;
        if (!getRamUtilizationHistory().isEmpty()) {
            int n = HISTORY_LENGTH;
            if (HISTORY_LENGTH > getRamUtilizationHistory().size()) {
                n = getRamUtilizationHistory().size();
            }
            double median = MathUtil.median(getRamUtilizationHistory());
            double[] deviationSum = new double[n];
            for (int i = 0; i < n; i++) {
                deviationSum[i] = Math.abs(median - getRamUtilizationHistory().get(i));
            }
            mad = MathUtil.median(deviationSum);
        }
        return mad;
    ...
    public double getRamUtilizationMean() {
        ...
        if (!getRamUtilizationHistory().isEmpty()) {
            ...
            for (int i = 0; i < n; i++) {
                mean += getRamUtilizationHistory().get(i);
            }
            mean /= n;
        }
        return mean * getMips();
    ...
    public double getRamUtilizationVariance() {
        double mean = getRamUtilizationMean();
        double variance = 0;
        if (!getRamUtilizationHistory().isEmpty()) {
            int n = HISTORY_LENGTH;
            if (HISTORY_LENGTH > getRamUtilizationHistory().size()) {
            ...
            for (int i = 0; i < n; i++) {
                double tmp = getRamUtilizationHistory().get(i) * getMips() - mean;
                variance += tmp * tmp;
            }
            variance /= n;
        }
    ...
    public double getBwUtilizationMad() {
        double mad = 0;
        if (!getBwUtilizationHistory().isEmpty()) {
            int n = HISTORY_LENGTH;
            if (HISTORY_LENGTH > getBwUtilizationHistory().size()) {
                n = getBwUtilizationHistory().size();
            }
            double median = MathUtil.median(getBwUtilizationHistory());
            double[] deviationSum = new double[n];
            for (int i = 0; i < n; i++) {
                deviationSum[i] = Math.abs(median - getBwUtilizationHistory().get(i));
            }
            mad = MathUtil.median(deviationSum);
        }
        return mad;
    ...
    public double getBwUtilizationMean() {
        double mean = 0;
        if (!getBwUtilizationHistory().isEmpty()) {
            int n = HISTORY_LENGTH;
            if (HISTORY_LENGTH > getBwUtilizationHistory().size()) {
            ...
            for (int i = 0; i < n; i++) {
                mean += getBwUtilizationHistory().get(i);
            }
            mean /= n;
        }
        return mean * getMips();
    ...
    public double getBwUtilizationVariance() {
        double mean = getBwUtilizationMean();
        double variance = 0;
        if (!getBwUtilizationHistory().isEmpty()) {
            int n = HISTORY_LENGTH;
            if (HISTORY_LENGTH > getBwUtilizationHistory().size()) {
            ...
            for (int i = 0; i < n; i++) {
                double tmp = getBwUtilizationHistory().get(i) * getMips() - mean;
                variance += tmp * tmp;
            }
            variance /= n;
        }
    ...
    public void addRamUtilizationHistoryValue(final double utilization) {
        getRamUtilizationHistory().add(0, utilization);
        if (getRamUtilizationHistory().size() > HISTORY_LENGTH) {
            getRamUtilizationHistory().remove(HISTORY_LENGTH);
        }
    }

    public void addBwUtilizationHistoryValue(final double utilization) {
        getBwUtilizationHistory().add(0, utilization);
    ...
    @Override
    public MyCloudletScheduler getCloudletScheduler() {
        return (MyCloudletScheduler) super.getCloudletScheduler(); //To change body of generated methods, choose Tools | Templates.
    }
